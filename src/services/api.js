...
class ApiService {
  ...
  async setupManagerPermissions() {
    try {
      console.log('🔧 API: Setting up manager permissions table...');
      const response = await this.api.post('/api/setup-manager-permissions');
      console.log('🔧 API: Setup completed:', response.data);
      return response.data;
    } catch (error) {
      console.error('🔧 API: Setup manager permissions error:', error);
      throw new Error('שגיאה בהקמת טבלת הרשאות מנהלים');
    }
  }

  async getManagerPermissions(managerId) {
    try {
      console.log('🔒 API: Getting permissions for manager:', managerId);
      const response = await this.api.get(`/api/manager-permissions/${managerId}`);
      console.log('🔒 API: Got manager permissions:', response.data);
      return response.data;
    } catch (error) {
      console.error('🔒 API: Get manager permissions error:', error);
      throw new Error('שגיאה בטעינת הרשאות מנהל');
    }
  }

  async updateManagerPermissions(managerId, permissions) {
    try {
      console.log('🔒 API: Updating permissions for manager:', managerId, permissions);
      const response = await this.api.put(`/api/manager-permissions/${managerId}`, permissions);
      console.log('🔒 API: Updated manager permissions:', response.data);
      return response.data;
    } catch (error) {
      console.error('🔒 API: Update manager permissions error:', error);
      throw new Error('שגיאה בעדכון הרשאות מנהל');
    }
  }
  ...
  async createUser(userData) {
    try {
      ...
      const serverData = {
        ...
        tipPercentage: parseInt(userData.tipPercentage) || 100
      };
  ...
  async updateEmployee(updateData) {
    try {
      ...
      // Send data exactly as server expects - with Hebrew encoding fixed
      const serverData = {
        employeeId: employeeId,
        firstName: fixHebrewText(firstName),
        lastName: fixHebrewText(lastName),
        role: role,
        realName: fixHebrewText(realName),
        realname: fixHebrewText(realName), // גם באותיות קטנות למקרה שהשרת מצפה לזה
        wname: `${fixHebrewText(firstName)} ${fixHebrewText(lastName)}`, // גם wname למקרה שזה מה שמעדכן
        working: working,
        aprove: aprove,
        tipPercentage: parseInt(tipPercentage) || 100
      };

      console.log('👤 API: Updating employee (Hebrew fixed):', employeeId, serverData);
