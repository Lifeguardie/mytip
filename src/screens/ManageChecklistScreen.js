...
const ManageChecklistScreen = ({ navigation }) => {
  ...
  const loadChecklist = async () => {
    try {
      const response = await api.getChecklist('morning');
  ...
  const saveChecklist = async (items) => {
    try {
      await api.saveChecklist('morning', items);
    } catch (error) {
      console.error('Error saving morning checklist:', error);
  ...
  return (
    <View style={styles.container}>
      <StatusBar style="light" backgroundColor="#FF9800" />
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.title}>ניהול צ'קליסט בוקר</Text>
        <TouchableOpacity 
...
export default ManageChecklistScreen;